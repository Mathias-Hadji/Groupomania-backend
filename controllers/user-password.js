...
module.exports = (req, res, next) => {
    ...
    if(req.body.userCurrentPassword.length == 0 || req.body.userNewPassword == 0){
    ...
    db.query(sql, function (err, result, fields) {
        ...
        const user = result[0]
        bcrypt.compare(req.body.userCurrentPassword, user.password_user)
        .then(valid => {

            if(!valid){
                return res.status(401).json({ notMatch: 'Le mot de passe que vous avez saisi est incorrect.' })
            }

            if(passwordValidator.validate(req.body.userNewPassword) == false){
